...
export function* addWidgetSaga(action) {
    try {
        yield axios.put(`${process.env.REACT_APP_API_SERVER_URL}api/v2/widgets/create`, action.payload)
...
export function* removeWidgetSaga(action) {
    try {
        yield axios.delete(`${process.env.REACT_APP_API_SERVER_URL}api/v2/widgets/${action.id}`)
